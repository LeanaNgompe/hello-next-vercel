...
export default function ItemsPage() {
  ...
  useEffect(() => {
    const fetchCaptions = async () => {
      ...
      if (error) {
        ...
        setError('Error loading captions.');
      } else {
        setCaptions(data || []);
      }
    };
  ...
  if (error && captions.length === 0) {
    return <div className="text-center text-gray-500 mt-10">{error}</div>;
  }
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {(!captions || captions.filter(c => c.images && c.images.url).length === 0) && (
